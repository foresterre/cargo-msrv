...
//  rust-version
pub fn parse_manifest_workaround<P: AsRef<Path>>(path: P) -> Option<semver::Version> {
...
pub(in crate::event) fn get_package_metadata_msrv(package: &Package) -> Option<semver::Version> {
    package
        .metadata
        .get("msrv")
        .and_then(|v| v.as_str())
