...
impl TerminateWithFailure {
    pub fn new(error: impl fmt::Display, highlight: bool) -> Self {
        Self {
            highlight,
            reason: SerializableReason {
...
mod tests {
    ...
    fn reported_non_is_not_error_event() {
        ...
        let event = TerminateWithFailure::new("Unable to print event output", false);
    ...
    fn reported_non_is_error_event() {
        ...
        let event = TerminateWithFailure::new(
            "Unable to find a Minimum Supported Rust Version (MSRV), for command 'cargo build --all'",
            true,
        );
